...
namespace Opc.Ua
{
    ...
    public class CertificateFactory
    {
        ...
        /// <param name="issuerKeyFilePassword">The  password for the PFX file containing the CA private key.</param>
        /// <returns></returns>
        public static X509Certificate2 CreateCertificateViaProxy(
        ...
        /// </summary>
        private static bool VerifyCRLSignature(X509Certificate2 issuer, byte[] crl, bool throwOnError)
        {
            if (issuer == null)
            {
            ...
            IntPtr pBuffer = Marshal.AllocHGlobal(crl.Length);
            Marshal.Copy(crl, 0, pBuffer, crl.Length);

            try
            {
                CERT_CONTEXT context = (CERT_CONTEXT)Marshal.PtrToStructure(issuer.Handle, typeof(CERT_CONTEXT));
                CERT_INFO info = (CERT_INFO)Marshal.PtrToStructure(context.pCertInfo, typeof(CERT_INFO));
                ...
                if (bResult == 0)
                {
                    ...
                    if (throwOnError)
                    {
                        throw ServiceResultException.Create(
                            ...
                            "Could not get verify signature on CRL. GetLastError={0}.",
                            error);
                    }
                }
            }
        ...
        /// <summary>
        /// Returns true if CRL was created by the issuer and the certificate is in it.
        /// </summary>
        public static bool IsCertificateInCRL(X509Certificate2 issuer, X509Certificate2 certificate, byte[] crl)
        {
            IntPtr pData = IntPtr.Zero;
            int dwDataSize = 0;
            IntPtr pData2 = IntPtr.Zero;
            int dwDataSize2 = 0;
            IntPtr pData3 = IntPtr.Zero;
            ...
            try
            {
                ...
                if (bResult == 0)
                {
                    ...
                    throw ServiceResultException.Create(
                        StatusCodes.BadCertificateInvalid,
                        "Could not decode the signed CRL. GetLastError={0}.",
                        error);
                }
                ...
                CERT_SIGNED_CONTENT_INFO info2 = (CERT_SIGNED_CONTENT_INFO)Marshal.PtrToStructure(pData, typeof(CERT_SIGNED_CONTENT_INFO));

                // calculate amount of memory required.
                bResult = Win32.CryptDecodeObjectEx(
                    X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
                    (IntPtr)X509_CERT_CRL_TO_BE_SIGNED,
                    info2.ToBeSigned.pbData,
                    info2.ToBeSigned.cbData,
                    CRYPT_DECODE_NOCOPY_FLAG,
                    IntPtr.Zero,
                    pData2,
                    ref dwDataSize2);
                ...
                if (bResult == 0)
                {
                    ...
                    throw ServiceResultException.Create(
                        StatusCodes.BadCertificateInvalid,
                        "Could not decode the CRL body. GetLastError={0}.",
                        error);
                }
                ...
                CRL_INFO info3 = (CRL_INFO)Marshal.PtrToStructure(pData2, typeof(CRL_INFO));

                // check if the CRL was issued by this certificate.
                string issuerName = null;
                Parse


            m_crl.X500Name(info3.Issuer, out issuerName);

                List<string> parsedIssuerName = Utils.ParseDistinguishedName(issuerName);
                parsedIssuerName.Reverse();

                if (!Utils.CompareDistinguishedName(issuer, parsedIssuerName))
                {
                    return false;
                }

                // verify the signature.
                VerifyCRLSignature(issuer, crl, true);

                // get the cert info for the target certificate.
                CERT_CONTEXT context4 = (CERT_CONTEXT)Marshal.PtrToStructure(certificate.Handle, typeof(CERT_CONTEXT));

                IntPtr[] pCRLs = new IntPtr[] { pData2 };
                pData3 = Marshal.AllocHGlobal(IntPtr.Size * pCRLs.Length);
                Marshal.Copy(pCRLs, 0, pData3, pCRLs.Length);
            ...
            finally
            {
                ...
                if (pData3 != IntPtr.Zero)
                {
                    Marshal.FreeHGlobal(pData3);
                    pData3 = IntPtr.Zero;
                }
            }
        }
        ...
        private static void DeleteKeyContainer(X509Certificate2 certificate)
        {
            ...
            int result = NativeMethods.CryptAcquireContextW(
                ref hProvider,
            ...
            result = NativeMethods.CryptAcquireContextW(
                ref hProvider,
                key.CspKeyContainerInfo.KeyContainerName,
                key.CspKeyContainerInfo.ProviderName,
                key.CspKeyContainerInfo.ProviderType,
